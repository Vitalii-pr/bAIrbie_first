...
def build_graph_from_note(path_to_file: str, graph=None):
    ...
    original_file_name = os.path.splitext(os.path.basename(path_to_file))[0]
    ...
    return {key: build_graph_from_note(value, {}) if isinstance(value, dict) else value for key, value in sorted(graph.items())}

def convert_to_dot(graph: dict) -> None:
    with open("graph.dot", 'w', encoding="utf-8") as f:
        f.write("digraph {\n")
        for point, values in sorted(graph.items()):
