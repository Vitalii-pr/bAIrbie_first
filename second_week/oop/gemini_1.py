...
class Word:
    ...
    def is_valid(self, board: 'Board') -> bool:
        ...
        return 4 <= len(word) <= 9 and board.grid[1][1] in word and \
               all(word.count(char) <= sum(board.grid, []).count(char) for char in set(word)) 
...
class Board:
    ...
    def get_words(self, filename: str) -> list['Word']:
        with open(filename, 'r', encoding="utf-8") as file:
            return [Word(word.strip().lower()) for word in file.readlines()[3:] if 4 <= len(word) <= 9 and Word(word).is_valid(self)]
...
class TargetGame:
    ...
    def get_user_words(self, val: list[str]) -> list[str]:
        ...
        try:
            return [input().strip().lower() for _ in iter(input, '') if len(_) >= 4] 
        except EOFError:
            return []  
